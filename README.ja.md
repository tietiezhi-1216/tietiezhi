...
| Tietiezhi コントロールセンター | 1 つのパネルで ID 指示、Markdown 長期メモリ、Skills / MCP の割り当て、ファイル形式のシークレット保管庫、Home ファイル、専用ツール権限を管理 |
...
3. 「Tietiezhi」で対象デバイスを選び、右上のコントロールセンターから ID、メモリ、Skills、MCP、Home ファイルを設定します。
4. モデルを同期・選択し、新しいタスクを作成します。ローカルリポジトリを扱う場合はプロジェクトフォルダーを選択します。
5. グローバル設定で再利用可能なエージェント、Skills、MCP を管理します。
