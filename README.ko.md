...
| Tietiezhi 제어 센터 | 하나의 패널에서 정체성 지침, Markdown 장기 기억, Skills / MCP 할당, 파일형 비밀 저장소, Home 파일, 전용 도구 권한 관리 |
...
3. **Tietiezhi**에서 대상 기기를 선택하고 오른쪽 위 제어 센터에서 정체성, 기억, Skills, MCP, Home 파일을 설정합니다.
4. 모델을 동기화하고 선택한 뒤 새 작업을 만듭니다. 로컬 저장소가 필요하면 프로젝트 폴더를 선택합니다.
5. 전역 설정에서 재사용 가능한 에이전트, Skills, MCP 서버를 관리합니다.
