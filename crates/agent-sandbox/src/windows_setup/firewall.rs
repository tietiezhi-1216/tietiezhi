...
const OFFLINE_BLOCK_RULE_NAME: &str = "tietiezhi_sandbox_offline_block_outbound";
const OFFLINE_BLOCK_LOOPBACK_TCP_RULE_NAME: &str = "tietiezhi_sandbox_offline_block_loopback_tcp";
const OFFLINE_BLOCK_LOOPBACK_UDP_RULE_NAME: &str = "tietiezhi_sandbox_offline_block_loopback_udp";

// Friendly text shown in the firewall UI.
const OFFLINE_BLOCK_RULE_FRIENDLY: &str = "Tietiezhi Sandbox Offline - Block Non-Loopback Outbound";
const OFFLINE_BLOCK_LOOPBACK_TCP_RULE_FRIENDLY: &str =
    "Tietiezhi Sandbox Offline - Block Loopback TCP (Except Proxy)";
const OFFLINE_BLOCK_LOOPBACK_UDP_RULE_FRIENDLY: &str =
    "Tietiezhi Sandbox Offline - Block Loopback UDP";
const OFFLINE_PROXY_ALLOW_RULE_NAME: &str = "tietiezhi_sandbox_offline_allow_loopback_proxy";
            ...
            let policy: INetFwPolicy2 = CoCreateInstance(&NetFwPolicy2, None, CLSCTX_INPROC_SERVER)
                .map_err(|err| {
                    anyhow::Error::new(SetupFailure::new(
                        SetupErrorCode::PolicyAccessFailed,
...
fn remove_rule_if_present(
    ...
    if unsafe { rules.Item(&name) }.is_ok() {
        unsafe { rules.Remove(&name) }.map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                SetupErrorCode::RuleCreateOrAddFailed,
...
fn validate_local_policy_modify_result(
    ...
    if result != S_OK {
        ...
        return Err(anyhow::Error::new(SetupFailure::new(
            SetupErrorCode::PolicyIneffective,
            format!(
...
fn configure_rule(rule: &INetFwRule3, spec: &BlockRuleSpec<'_>) -> Result<()> {
    ...
    let actual = unsafe { rule.LocalUserAuthorizedList() }.map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            SetupErrorCode::RuleVerifyFailed,
...
mod tests {
    ...
    fn local_policy_modify_state_rejects_ineffective_policy() {
        ...
        assert_eq!(failure.code, SetupErrorCode::PolicyIneffective);
    }
