...
| R12 | Patch 与 Diff | 已完成 | crates/agent-patch Lark/流式解析与多文件原子事务；crates/agent-approval FileChange 反向请求；agent-tools apply_patch；agent-core FileChange/Turn Diff；docs/CODEX-PATCH.md | apply_patch intentionally rejects binary payloads and preserves unrelated user changes; platform enforcement is delegated to the completed R15-R17 sandbox layers. |
...
| R14 | 审批策略 | 已完成 | crates/agent-approval 策略、精确会话缓存和持久 Amendment；agent-tools request_permissions；desktop V2 路由与 CodexApprovalPrompt；docs/CODEX-APPROVAL.md | Approval policy never widens the OS sandbox by itself; permanent grants remain explicit policy amendments while acceptForSession stays exact and Thread-scoped. |
| R15 | macOS 沙箱 | 已完成 | crates/agent-sandbox 实现固定 /usr/bin/sandbox-exec、read-only/workspace-write/danger/external、可写根、受保护元数据、临时目录、网络开关和 symlink 安全；agent-exec Pipe/PTY、Unified Exec、command/exec 与 apply_patch 共用策略；10 项沙箱测试、7 项 exec 测试和 20 项工具测试覆盖真实 macOS 拒绝。 | R21 provides the layered Permission Profile configuration; macOS network isolation remains tied to the managed proxy listener lifecycle. |
...
| R17 | 网络策略 | 已完成 | crates/agent-network attributed proxy; crates/agent-sandbox elevated Offline/Online identity, DPAPI, Firewall and persistent WFP; agent-tools/Unified Exec integration; Windows proxy-only escape test; desktop App Server setup/readiness; docs/CODEX-NETWORK.md | Windows setup requires administrator consent and fails closed when enterprise policy prevents local Firewall/WFP rules; proxy port drift is reconciled before execution. |
| R18 | ExecPolicy | 已完成 | crates/agent-absolute-path; crates/agent-shell-command; crates/agent-execpolicy; crates/agent-tools dynamic pre-spawn policy; desktop persisted rules/default.rules; docs/CODEX-EXECPOLICY.md; 26 absolute-path, 141 shell, 13 execpolicy and 23 tool tests | R21 provides layered project/profile policy discovery and Requirements enforcement; ExecPolicy itself remains advisory to approval and cannot bypass the completed OS sandbox. |
| R19 | MCP 完整实现 | 已完成 | crates/agent-mcp; desktop MCP App Server V2 dispatch and DesktopMcpHost; agent-core mcpToolCall persistence; source-built stdio interoperability fixture; Elicitation SSR test; docs/CODEX-MCP.md | OAuth discovery and refresh use rmcp 2.2.0 behind a mutex and OS credential store; R24 supplies plugin-owned MCP lifecycle. |
